...
sub main {
	...
	my $inputs = initialize_parameters();

	# load the reference sequence
	my $ref_string = load_reference_sequence($inputs);

	# mutate the reference
	mutate_reference($inputs, \$ref_string);

	# generate the reads
	generate_reads($inputs, \$ref_string);
